# 학습된 UNet 모델의 Test set 평가 스크립트 (추론 속도 포함)
# - Test set에 대해 Loss, IoU, Dice, Precision, Recall, Pixel Accuracy 산출
# - GPU inference time 측정 (mean / p95)
# - 결과: 모델 폴더에 evaluation_result.txt 저장
...
# =========================
# 4) Loss 함수 정의 (bead_train_end.py와 동일)
# =========================
...
# =========================
# 5) 평가 (Loss + IoU/Dice/Precision/Recall/Accuracy + inference time)
# =========================
...
with torch.no_grad():
    for images, masks in tqdm(test_loader, desc="Test evaluating"):
        ...
        # Loss 계산
        loss_val  = total_loss(logits, masks).item()
...
result_lines = [
    ...
    "------------------------------",
    f"📊 Test Total Loss      : {avg_loss:.4f}  (BCE: {avg_bce:.4f} + Dice: {avg_dice_loss:.4f})",
