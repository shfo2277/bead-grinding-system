#!/usr/bin/env python3
"""
비드 세그멘테이션 노드 (가공 전 인식) — bead_unet_node

[기능]
  카메라 컬러 이미지를 실시간으로 구독하여 UNet 모델로 용접 비드를
  세그멘테이션한다. 안정적인 프레임을 자동 선정한 뒤, 비드 외곽선(contour)과
  폭(width) 데이터를 CSV로 저장하고, 마스크/오버레이 이미지를 퍼블리시한다.

[모델]
  - UNet (resnet34 인코더), 입력 1024x1024, torchvision 전처리
  - 가중치: modelresult/1125unet.pth
  - 임계값: 0.5

[안정 프레임 선정]
  - Warmup 1초 (카메라 AE/AWB 안정화)
  - 마스크 면적 >= 150px, 최대 contour 면적 >= 120px 이면 "good"
  - 연속 3프레임 good → 확정 (또는 8초 타임아웃 시 best 후보 강제 확정)
  - 확정 후 카메라 구독 해제 (GPU 절약)

[ROS2 구독 토픽]
  - /camera/camera/color/image_rect_raw (sensor_msgs/Image, RELIABLE)

[ROS2 발행 토픽]
  - /bead/mask    (sensor_msgs/Image, BEST_EFFORT) : 흑백 마스크
  - /bead/overlay (sensor_msgs/Image, BEST_EFFORT) : 오버레이 시각화
  ※ 확정 후 1초마다 재발행 (RViz/브릿지 놓침 방지)

[저장 파일]
  - csv_latest/bead_contour.csv : 비드 외곽선 좌표 (x,y)
  - csv_latest/bead_width.csv   : 비드 폭 5구간 측정값 (slice_y, x_start, width_px)
  - 세션/recognition/ 폴더에도 CSV 복사 저장
  - 이미지 7종: 마스크, 원본, 오버레이, 외곽선, 폭선, PCA경로, 외곽선only
"""
...
# ===== contour / width 저장 =====
CSV_DIR     = "/workspace/BEADtrain/csv_latest"
CONTOUR_CSV = os.path.join(CSV_DIR, "bead_contour.csv")
WIDTH_CSV   = os.path.join(CSV_DIR, "bead_width.csv")
NUM_WIDTH_SLICES = 5
...
class BeadUNetNode(Node):
    ...
    def compute_and_save_contour_width_once(self, mask_resized: np.ndarray):
        ...
        with open(WIDTH_CSV, "w") as f:
            ...
            for y_line in ys:
                ...
                if len(xs) >= 2:
                    ...
                    x_max = max(xs)
                    width = x_max - x_min
                    line = f"{int(y_line)},{int(x_min)},{int(width)}\n"
                    f.write(line)
                    width_lines.append(line)
                    self.width_slices_y.append(int(y_line))

        # recognition 폴더에도 CSV 복사 저장
        session_dir = os.environ.get('GRIND_SESSION_DIR')
        if session_dir:
            recog_dir = os.path.join(session_dir, "recognition")
        else:
            recog_dir = None
        ...
        if recog_dir:
            ...
            np.savetxt(
                os.path.join(recog_dir, "bead_contour.csv"),
                bead_contour.reshape(-1, 2),
                delimiter=",",
                header="x,y",
                comments=""
            )
            with open(os.path.join(recog_dir, "bead_width.csv"), "w") as f:
                f.writelines(width_lines)
            self.get_logger().info(f"📐 CSV also saved to: {recog_dir}")
