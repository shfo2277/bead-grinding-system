#!/usr/bin/env python3
"""
3D 그라인딩 경로 생성 노드 — bead_pose_node

[기능]
  bead_unet_node가 생성한 2D 마스크와 aligned depth 이미지를 합쳐서
  로봇 world 좌표계의 3D 그라인딩 경로(웨이포인트)를 생성한다.
  TF lookup 대신 고정 4x4 변환 행렬을 사용한다 (스캔 위치 고정 전제).

[변환 행렬 (하드코딩)]
  - T_world_tool0 : 스캔 위치 Flange 좌표 (joints: 80.14, 43.01, 80.39, -16.98, -30.36, 13.99)
  - T_tool0_cam   : Hand-Eye 캘리브레이션 결과 (TSAI, 35 samples, 2026-02-26)
  - T_world_cam = T_world_tool0 @ T_tool0_cam

[처리 흐름]
  1. 마스크 픽셀 (u,v) + depth → 카메라 3D 좌표 (pinhole model)
  2. depth 이상값 필터 (median ± 3*MAD)
  3. 카메라 좌표 → world 좌표 (T_world_cam 행렬 변환)
  4. XY 평면 PCA → 비드 주방향 계산
  5. PCA 직선 따라 30개 웨이포인트 균등 샘플링 (Z = median)

[ROS2 구독 토픽]
  - /camera/camera/color/camera_info              (sensor_msgs/CameraInfo, RELIABLE) : 카메라 내부 파라미터
  - /camera/camera/aligned_depth_to_color/image_raw (sensor_msgs/Image, RELIABLE)     : aligned depth
  - /bead/mask                                     (sensor_msgs/Image, BEST_EFFORT)   : bead_unet_node 마스크

[ROS2 발행 토픽]
  - /bead/grind_path_base   (nav_msgs/Path)              : 3D 경로 (world 좌표, m)
  - /bead/grind_start_pose_base (geometry_msgs/PoseStamped) : 경로 시작점
  - /bead/grind_end_pose_base   (geometry_msgs/PoseStamped) : 경로 끝점
  - /bead/start_marker      (visualization_msgs/Marker)   : 시작점 RViz 마커
  - /bead/end_marker         (visualization_msgs/Marker)   : 끝점 RViz 마커
  - /bead/points_base        (sensor_msgs/PointCloud2)     : 비드 포인트클라우드 (RViz 시각화)
  ※ Path는 확정 후 1초마다 재발행

[ROS2 서비스]
  - reset_bead_path (std_srvs/Trigger) : 경로 잠금 해제 (재계산 허용)

[저장 파일]
  - bead_waypoints.csv : 웨이포인트 좌표 (x_mm, y_mm, z_mm)
"""
import os
import csv
import math
