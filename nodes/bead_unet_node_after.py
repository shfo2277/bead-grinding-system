#!/usr/bin/env python3
"""
bead_unet_node_after.py — 그라인딩 후 비드 제거 판정 노드 (Post-Grinding Judgment Node)

[주제] 그라인딩 후 카메라 영상을 UNet으로 세그멘테이션하여, 기준 외곽선(contour) 대비
       비드 제거 진행률을 실시간 판정하고 STOP 신호를 발행하는 ROS2 노드.

[모델]
  - UNet (resnet34 encoder, segmentation_models_pytorch)
  - 입력: 1280×1280 (LongestMaxSize + PadIfNeeded, albumentations 전처리)
  - 출력: 이진 마스크 (sigmoid > 0.4)
  - 가중치: /workspace/BEADtrain/modelresult/2026-03-07_23-23-27/unet_best.pth
    (중간 그라인딩 시편 추가 학습 모델)

[판정 로직]
  - shape_match_ratio: 모델 마스크와 기준 contour 영역의 겹침 비율 계산
  - shape_ok: shape_ratio >= SHAPE_THRESH (0.93) 이면 OK
  - overcut 감지: 기준 contour 외부에서 비드가 인식되면 overcut → OK
  - 최종 판정: ok = shape_ok OR has_overcut → Bool(/stop) 발행 (1회)

[오버레이 색상]
  - 녹색 (0,255,0): grinding — 모델 인식 AND 기준 contour 내부 (정상 제거 영역)
  - 주황색 (0,165,255): residual — 기준 내부 AND 모델 미인식 (잔여 비드)
  - 빨강색 (0,0,255): overcut — 모델 인식 AND 기준 외부 (과삭)

[Subscribe 토픽]
  - /camera/camera/color/image_rect_raw (sensor_msgs/Image, BEST_EFFORT)
    : RealSense D405 컬러 영상 (1280×720)

[Publish 토픽]
  - /grinding/mask    (sensor_msgs/Image, mono8)       : UNet 세그멘테이션 마스크
  - /grinding/raw     (sensor_msgs/Image, bgr8)        : 원본 카메라 영상
  - /grinding/overlay (sensor_msgs/Image, bgr8)        : 색상 오버레이 + 판정 텍스트
  - /grinding/overlay/compressed (sensor_msgs/CompressedImage, jpeg)
    : 오버레이 JPEG 압축 (Humble↔Jazzy 호환용)
  - /stop (std_msgs/Bool)                              : 그라인딩 정지 신호 (1회 발행)

[저장 파일]
  - {session_dir}/scan_{N}/raw.png           : 원본 영상
  - {session_dir}/scan_{N}/overlay_color.png : 색상 오버레이 (텍스트 없음)
  - {session_dir}/scan_{N}/overlay_judge.png : 색상 오버레이 + 판정 텍스트
  - /workspace/BEADtrain/scan_result_latest.txt : 최신 스캔 결과 (tcp_test_client_V3에서 읽어감)
  * session_dir: 환경변수 GRIND_SESSION_DIR (tcp_test_client_V3가 설정)
  * scan_N: 환경변수 GRIND_SCAN_NUM (tcp_test_client_V3가 설정)

[기준 데이터]
  - /workspace/BEADtrain/csv_latest/bead_contour.csv : 기준 외곽선 좌표
  - /workspace/BEADtrain/csv_latest/bead_width.csv   : 기준 폭 데이터 (현재 미사용)
"""
...
#중간 그라인딩 시편 추가해 학습한 모델
MODEL_PATH = "/workspace/BEADtrain/modelresult/2026-03-07_23-23-27/unet_best.pth"

CONTOUR_CSV = "/workspace/BEADtrain/csv_latest/bead_contour.csv"
WIDTH_CSV   = "/workspace/BEADtrain/csv_latest/bead_width.csv"
...
class BeadStopJudgeNode(Node):
    ...
    def image_callback(self, msg: Image):
        ...
        # ---- STOP JUDGE (contour only + overcut) ----
        ...
        cv2.drawContours(ref_mask, [self.ref_contour], -1, 255, -1)

        mask_bool = mask > 0
        ref_bool = ref_mask > 0

        # 녹색: grinding = 모델 인식 AND 기준 내부
        grinding = mask_bool & ref_bool
        # 주황: residual = 기준 내부 AND 모델 미인식
        ...
        will_stop = (ok and not self.stop_published)
        ...
        # 색상 오버레이만 (텍스트 없음) — 이미지 저장용
        overlay_clean = overlay.copy()
        ...
        if has_overcut:
            put_label(overlay, f"Overcut : detected", (20, 120))
        ...
        if not self.image_saved:
            ...
            cv2.imwrite(os.path.join(save_dir, "overlay_color.png"), overlay_clean)
            cv2.imwrite(os.path.join(save_dir, "overlay_judge.png"), overlay)
        ...
        if now - self.last_log_t >= 1.0:
            ...
            self.get_logger().info(
                f"✅ SHAPE OK = {shape_ratio >= SHAPE_THRESH}"
            )
