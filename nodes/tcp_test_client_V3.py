...
[기능]
  전체 자동 그라인딩 루프를 관리하는 메인 프로그램.
  로봇 TCP 소켓 4개를 통해 Staubli RX160 로봇을 제어하고,
  bead_unet_node / bead_pose_node / bead_unet_node_after를
  subprocess로 실행/종료하며 그라인딩 사이클을 반복한다.

[자동 그라인딩 루프 (CMD 'a')]
  1. CMD 2 → 로봇을 스캔 위치로 이동
  2. (첫 사이클만) bead_unet_node 실행 → 비드 인식 → contour/width CSV 저장
  3. (첫 사이클만) bead_pose_node 실행 → 3D 경로 생성 (Path 메시지 수신)
     (2회차부터는 첫 경로 재사용 + Z축 -0.5mm씩 오프셋)
  4. CMD 3 → 웨이포인트를 Motion 소켓으로 로봇에 전송 + 가공 준비
  5. CMD 4 → 실제 그라인딩 실행
  6. CMD 2 → 스캔 위치 복귀
  7. bead_unet_node_after 실행 → 비드 잔여 판정
     → OK(STOP 수신) → CMD 1 → 홈 복귀 → 종료
     → NOK → 다음 사이클 반복

[TCP 소켓]
  - Command  (11001): 명령 전송 (1=홈, 2=스캔위치, 3=가공준비, 4=가공시작)
  - Motion   (11000): 웨이포인트 좌표 전송 (float 배열)
  - Feedback (11002): 로봇 상태 수신 (0~4)
  - Joint    (11003): 조인트 값 수신

[ROS2 구독 토픽]
  - /bead/grind_path_base (nav_msgs/Path)     : bead_pose_node가 생성한 3D 경로
  - /stop                 (std_msgs/Bool)      : bead_unet_node_after의 STOP 신호

[저장 파일]
  - bead_waypoints.csv      : 웨이포인트 좌표 (mm)
  - grinding_log.txt        : 그라인딩 세션 로그
  - scan_result_latest.txt  : 최신 판정 결과 (after 노드에서 기록)
"""
...
CSV_DIR = os.path.join(WORKSPACE, 'csv_latest')
...
def start_node(script_name, extra_env=None):
    """BEADtrain/nodes 폴더에서 Python 노드 실행"""
    cmd = (
        f"source /opt/ros/humble/setup.bash && "
        f"export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && "
        f"cd {WORKSPACE}/nodes && python3 -u {script_name}"
    )
    env = os.environ.copy()
    if extra_env:
